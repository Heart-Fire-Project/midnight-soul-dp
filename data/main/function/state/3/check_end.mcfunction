...
execute if score $shard_collect data = $goal temp run function main:state/4/enter

# 若仅剩一位灵魂，20 秒后进灵气 4
execute if score $alive data matches 1 if score $3_gametime countdown matches ..17599 run scoreboard players set $3_gametime countdown 17599

# 若已经不能再拖
execute if score $3_gametime countdown matches 19200.. run function main:state/4/enter