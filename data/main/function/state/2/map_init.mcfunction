...
# 设置地图信息
execute if score $map data matches 1 run data merge storage ms:map {shard_goal:24s,shard_summon:30s,chest_summon:5s,chest_max:12s}
execute if score $map data matches 2 run data merge storage ms:map {shard_goal:18s,shard_summon:21s,chest_summon:4s,chest_max:10s}
execute if score $map data matches 3 run data merge storage ms:map {shard_goal:40s,shard_summon:48s,chest_summon:6s,chest_max:15s}
...
execute if score $echo data matches 3 run scoreboard players set $summon temp 1
