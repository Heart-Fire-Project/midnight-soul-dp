# 是否新纪录?
execute if score @s exp_temp > @s stat.single_record run tellraw @s [{"text":"» ","bold":true,"color":"gold"},{"translate":"ms.info.new_singal_record","fallback":"新表现分纪录！","bold":false}]
scoreboard players operation @s stat.single_record > @s exp_temp
...
scoreboard players operation @s stat.rating += @s exp_temp
scoreboard players operation $temp temp -= @s stat.rating
