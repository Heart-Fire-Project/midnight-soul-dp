...
tellraw @s[scores={item=2}] [{"translate":"ms.item.02.desc","fallback":"将周围 7.5 格内的灵魂碎片聚集至脚下；重叠的碎片可同时收集"}]
...
tellraw @s[scores={item=6}] [{"translate":"ms.item.06.desc","fallback":"生效期间内守卫者不受重力影响"}]