...
$tellraw @s[scores={talent_$(num)=1}] [{"translate":"ms.talent.001.desc","fallback":"收集灵魂碎片后，移速 +40%；下一次收集时直接获取 0.5 秒的进度，收集中断不会返还该效果"}]
$tellraw @s[scores={talent_$(num)=2}] [{"translate":"ms.talent.002.desc","fallback":"周围 12 格内有守卫者时，移速 +80%"}]
$tellraw @s[scores={talent_$(num)=3}] [{"translate":"ms.talent.003.desc","fallback":"当有灵魂处于垂死状态时，移速 +20%"}]
