...
tellraw @s[scores={skill=4}] [{"translate":"ms.skill.004.desc","fallback":"有守卫者距离 20 格以内时可能触发自身移速 +100% 且守卫者移速 -90%，或是自身移速 -60%；若守卫者距离 20 格以外则可能触发使最近的守卫者发光且强制进入擦刀状态，或是自身传送至最近的守卫者处并移速 -100%；触发上述负面效果的概率初始为 5%，但每次使用该技能都会 +7%，至多提升至 40%"}]
